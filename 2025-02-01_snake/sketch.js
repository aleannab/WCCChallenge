...
let gPalette = ['#fec707', '#a22a20', '#0c0c0c'];
let gBgColor = '#f1f3f4';
...
function draw() {
  ...
  for (let i = 0; i < gCircles.length; i++) {
    curveVertex(gCircles[i].x, gCircles[i].y);
  }
  endShape();
}
...
function createPathCircleRefs() {
  ...
  let rowCount = 5; //int(random(5, 7));
  let vSpacing = 5 * gStrokeWeight;
  ...
  for (let row = 0; row < rowCount; row++) {
    ...
    isLeft = !isLeft;
    let hSpacingMax = 105000;
    let hSpacing = isLeft ? -hSpacingMax : hSpacingMax;
    for (let col = 0; col < 2; col++) {
      let circlePos = checkBounds(createVector(col * 0.75 * width, yp));
